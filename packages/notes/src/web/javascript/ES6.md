...
```js
javascriptNumber.prototype[Symbol.iterator] = function* () {
  let i = 0;
...
```javascript
let arrayLike = { 0: "a", 1: "b", 2: "c", length: 3 }; // TypeError: Cannot spread non-iterable object.
let arr = [...arrayLike];
```
...
实际应用中，常见类似数组的对象是 DOM 操作返回的 NodeList 集合，以及函数内部的 arguments 对象。 Array.from 都可以将它们转为真正的数组。
...
这个方法的主要目的，是弥补`数组构造函数` Array() 的不足。因为参数个数的不同，会导致 Array() 的行为有差异。
...
下面代码用来检查当前环境是否支持该方法，如果不支持，部署一个简易替代版本。
